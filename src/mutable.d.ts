...
export type Mutable<BaseType, Keys extends keyof BaseType = keyof BaseType> = Simplify<
    ...
    Except<BaseType, Keys> & { -readonly [KeyType in keyof Pick<BaseType, Keys>]: Pick<BaseType, Keys>[KeyType] } // Pick the keys that should be mutable from the base type and make them mutable by removing the `readonly` modifier from the key.
>;
